...
func newUnauthenticatedClient(ctx context.Context, cfg *unauthenticatedClientConfig) (*Client, error) {
	...
	if len(rawClusterCAs) == 0 {
		// If given a proxy address the client will ask the proxy to use ALPN
		// SNI routing to forward the connection to the auth server. Fetch the
	...
	// succesfully got CA certs above, auth server is verified, return this
	// client
	if !tlsConfig.InsecureSkipVerify {
		return unauthenticatedClient, nil
	}

	// if we couldn't get certs from disk or proxy and no CA pins were given
	...
	// we're done with this client, will create a new one which verifies the TLS
	// cert
