...
The exact set of required permissions depends on whether you're using RDS
instances or Aurora clusters (or both), as well as the IAM identity your
Teleport database agent will be using (user or role).

<Details title="Example full IAM policy document" opened={false}>
```json
{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Effect": "Allow",
            "Action": [
                "rds:DescribeDBInstances", // Only for RDS instances
                "rds:ModifyDBInstance",    // Only for RDS instances
                "rds:DescribeDBClusters",  // Only for Aurora clusters
                "rds:ModifyDBCluster",     // Only for Aurora clusters
                "iam:GetUserPolicy",       // Only when running as IAM user
                "iam:PutUserPolicy",       // Only when running as IAM user
                "iam:DeleteUserPolicy",    // Only when running as IAM user
                "iam:GetRolePolicy",       // Only when running as IAM role
                "iam:PutRolePolicy",       // Only when running as IAM role
                "iam:DeleteRolePolicy"     // Only when running as IAM role
            ],
            "Resource": "*"
        }
    ]
...
Make sure to keep only the permissions needed for your use-case.
...
<Details title="Example full IAM permission boundary document" opened={false}>
```json
{
    "Version": "2012-10-17",
    "Statement": [
        {
            ...
            "Action": [
                ...
                "iam:PutRolePolicy",       // Only when running as IAM role
                "iam:DeleteRolePolicy",    // Only when running as IAM role
                "rds-db:connect"           // Should always be present
            ],
            "Resource": "*"
        }
    ]
...
Similarly to the IAM policy, only keep permissions needed for your use-case.
